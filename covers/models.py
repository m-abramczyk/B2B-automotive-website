...
def upload_to_cs(instance, filename):
    return f'covers_cs/{filename}'
...
class Cover(models.Model):
    ...
    class Meta:
        verbose_name = ('Page Cover')
        verbose_name_plural = ('Page Covers')
...
#//////////////////////////////////////////////////////////////
# Case Study Covers
class CaseStudyCover(models.Model):

    # Cover Video
    cover_video = models.FileField(
        upload_to=upload_to_cs,
        null=True,
        blank=True,
        validators=[FileExtensionValidator(allowed_extensions=['mp4', 'webm'])],
        help_text=('WEBM / MP4 file, recommended max size: 4Mb / length: ~30s. Upload either Video or Image Cover, not both.'),
